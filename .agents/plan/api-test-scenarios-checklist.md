...
- [x] POST /v0/images/ocr | 권한 없음 (GlobalUnauthorizedTest)
...
- [x] GET /my | page 최소 위반
- [x] GET /my | 인증 없이 접근 (GlobalUnauthorizedTest)
- [x] POST / (addBook) | 새 도서 + 책장 추가
- [x] POST / | 동일 도서 중복 추가
- [x] POST / | 잘못된 ISBN 등 유효성
- [x] POST / | 인증 없이 추가 시도 (GlobalUnauthorizedTest)
...
- [x] GET /my/count·/my/search | 인증 없이 접근 (GlobalUnauthorizedTest)
...
- [x] PATCH·DELETE 보호 엔드포인트 | 인증 헤더 누락 (GlobalUnauthorizedTest)
...
- [x] POST/GET 보호된 카드 API | 인증 없이 호출 (GlobalUnauthorizedTest)
...
- [x] PATCH·DELETE /{cardId} | 인증 없이 접근 (GlobalUnauthorizedTest)
...
- [x] GET /{readingId} | 비공개 독서 + 비로그인 (GlobalUnauthorizedTest)
...
- [x] PATCH /{cardId}/visibility | 인증 없이 요청 (GlobalUnauthorizedTest)

### PostController (`/api/v1/posts`)
- [x] POST / | 게시글 작성
- [x] POST / | 이미지 11장 이상
- [x] POST / | bookId null
- [x] POST / | 인증 없이 작성 시도 (GlobalUnauthorizedTest)
- [x] GET /my | 나의 게시글 목록
- [x] GET /my | 게시글 없음
- [x] GET /my | 인증 없이 접근 (GlobalUnauthorizedTest)
- [x] GET / | 전체 feed 최신순
...
- [x] POST /reply | 인증 없이 작성 (GlobalUnauthorizedTest)
- [x] PATCH /{commentId} | 본인 댓글 수정
- [x] PATCH /{commentId} | 타인 댓글 수정 시도
- [x] PATCH /{commentId} | 인증 없이 수정 시도 (GlobalUnauthorizedTest)
- [x] DELETE /{commentId} | 소유 댓글 삭제(soft delete)
- [x] DELETE /{commentId} | 이미 삭제된 댓글 재삭제
- [x] DELETE /{commentId} | 인증 없이 삭제 시도 (GlobalUnauthorizedTest)
